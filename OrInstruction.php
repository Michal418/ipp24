...
class OrInstruction extends Instruction {
    ...
    public function __construct(Argument $var, Argument $symb1, Argument $symb2)
    {
        if ($var->getIppType() !== IPPType::VAR || !IPPType::isVarOrData($symb1->getIppType()) || !IPPType::isVarOrData($symb2->getIppType())) {
            throw new InvalidArgumentException("Invlaid arguments for OR: {$var}, {$symb1}, {$symb2}");
        }

        parent::__construct('OR', [$var, $symb1, $symb2]);
    }
    ...
    public function execute(Interpreter $interpreter) : void {
        $interpreter->or($this->getArguments()[0]->getText(), $this->getArguments()[1], $this->getArguments()[2]);
    }
};
